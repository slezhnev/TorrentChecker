...
import ru.lsv.torrentchecker.server.torrents.impl.RutrackerDownloader;
...
public class TorrentsDownloader extends DownloaderAbstract {
	...
	public TorrentsDownloader() {
		...
		try {
			downloaders.add(new RutrackerDownloader(httpContext));
		} catch (DownloaderException e) {
			// Do nothing
		}
	}
	 ...
	 */
	@Override
	public List<FileResult> check(List<File> torrentFiles,
		...
		for (File torrentFile : torrentFiles) {
			...
			if (!decoded.containsKey("comment")) {
				...
				try {
					...
					// Добавляем FAIL и переходим к следующему
					results.add(new FileResult(torrentFile.getName(),
							"Exception while getting \"comment\" from torrent file"));
					continue;
				} catch (MalformedURLException e) {
					// Добавляем FAIL и переходим к следующему
					results.add(new FileResult(torrentFile.getName(),
							"Malformed URL in comment"));
					continue;
				}
				...
				// downloader'а
				Optional<TorrentDownloaderAbstract> implDownloader = downloaders
						.stream()
						.filter(impl -> impl.getResource().equals(torrentHost))
						.findFirst();
				...
				if (!preparedDownloaders.containsKey(implDownloader.get())) {
					...
					try {
						if (implDownloader.get().authenticate(torrentUrl,
								user.getUserName(), user.getPassword())) {
							// Добавляем к приготовленным
							DownloaderInProcess downloads = new DownloaderInProcess();
							downloads.downloads.add(new TorrentInProcess(
									torrentFile, torrentUrl, decoded));
							preparedDownloaders.put(implDownloader.get(),
									downloads);
						}
					} catch (DownloaderException e) {
						// Добавляем FAIL и переходим к следующему
						results.add(new FileResult(torrentFile.getName(), e
								.getMessage()));
						continue;
					}
				} else {
					// Добавляем URL и файл к скачиванию
					preparedDownloaders.get(implDownloader.get()).downloads
							.add(new TorrentInProcess(torrentFile, torrentUrl,
									decoded));
				}
			}
		}
		// Поехали по подготовленным к загрузке downloader'ам
		for (TorrentDownloaderAbstract implDownloader : preparedDownloaders
				.keySet()) {
			// Поехали по URL
			for (TorrentInProcess downloadInProcess : preparedDownloaders
					.get(implDownloader).downloads) {
				logger.info("Torrent checker - Processing "
						+ downloadInProcess.file.getName());
				// Нашли имплементацию. Загружаем файлО во временное
				// место хранения
				String downloadedTorrent = null;
				try {
					downloadedTorrent = implDownloader.downloadTorrent(
							downloadInProcess.downloadURL, pathToDownload);
				} catch (DownloaderException e) {
					// Добавляем FAIL и переходим к следующему
					results.add(new FileResult(
							downloadInProcess.file.getName(), e.getMessage()));
					continue;
				}
				if (downloadedTorrent != null) {
					...
					if (downloadedTorrentFile.exists()) {
						...
						try {
							fileCompare = org.apache.commons.io.FileUtils
									.checksumCRC32(downloadInProcess.file) != org.apache.commons.io.FileUtils
									.checksumCRC32(downloadedTorrentFile);
						} catch (Exception e1) {
							// Добавляем FAIL и переходим к следующему
							results.add(new FileResult(downloadInProcess.file
									.getName(), "Failed on CRC32 check"));
							continue;
						}
						if (fileCompare || (!isFileInQueue)) {
							...
							// там нового
							Optional<Set<String>> filesInTorrent = getFilesFromDecoded(
									downloadInProcess.file,
									downloadInProcess.decoded);
							...
							try {
								...
								results.add(new FileResult(
										downloadInProcess.file.getName(),
										"Failed on accessing downloaded torrent file"));
								continue;
							}
							Map<String, BEValue> decodedDownloaded = null;
							try {
								decodedDownloaded = BDecoder
										.bdecode(torrentFIS).getMap();
								torrentFIS.close();
							} catch (Exception e1) {
								results.add(new FileResult(
										downloadInProcess.file.getName(),
										"Failed on decoding torrent"));
								continue;
							}
							// Получаем список файлов из него
							Optional<Set<String>> downloadedTorrentFiles = getFilesFromDecoded(
									downloadedTorrentFile, decodedDownloaded);
							if (filesInTorrent.isPresent()
									&& downloadedTorrentFiles.isPresent()) {
								downloadedTorrentFiles
										.get()
										.stream()
										.filter(file -> !filesInTorrent.get()
												.contains(file))
							...
							if ((!isFileInQueue) || (newFiles.size() > 0)) {
								...
								try {
									...
									results.add(new FileResult(
											downloadInProcess.file.getName(),
											"Failed copying to \"autoload\""));
									continue;
								}
								// Копирование в inqueue выключено - ибо сейчас
								// работа будет идтить только с inqueue
								// torrentsPath больше НЕ используется
								/*
								 * try { // И копируем его в inqueue
								 * org.apache.commons
								 * .io.FileUtils.copyFile(downloadInProcess
								 * .file, new File(Commons.getTorrentsInQueue()
								 * + downloadInProcess.file.getName())); } catch
								 * (IOException e) { // Добавляем FAIL и
								 * переходим к // следующему results.add(new
								 * FileResult(downloadInProcess.file.getName(),
								 * "Failed copying to \"inqueue\"")); continue;
								 * }
								 */
							...
							results.add(new FileResult(downloadInProcess.file
									.getName(), FileProcessingResult.UNCHANGED));
						}
						...
						results.add(new FileResult(downloadInProcess.file
								.getName(),
								"Torrent downloaded, but does not exist o_O"));
						continue;
					}
				} else {
					results.add(new FileResult(
							downloadInProcess.file.getName(),
							implDownloader.getClass().getName()
	 ...
	 *         найти не удалось
	 */
	public Optional<Set<String>> getFilesFromDecoded(File torrentFile,
			Map<String, BEValue> decoded) {
		...
		if (decoded.containsKey("files")) {
			...
			// А если мы доехали до сюда - то тогда тут ничего нету. Возвращаем
			// пустоту
			return Optional.empty();
		}
	...
	private class TorrentInProcess {
		 ...
		 *            URL для скачивания
		 */
		public TorrentInProcess(File file, URL downloadURL,
				Map<String, BEValue> decoded) {
			super();
			this.file = file;
